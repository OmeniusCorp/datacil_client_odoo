...
class TestPurchaseOrderDatacil(DatacilTestCommon):
    ...
    def test_check_on_unsaved_document_uses_identification(self):
        ...
        with self.mock_get([self.ok(RUC_PAYLOAD), self.ok(CREDITS_PAYLOAD)]):
            result = self.env['purchase.order'].datacil_check_partner('1790016919001')
        self.assertTrue(result['success'])
        self.assertEqual(partner.datacil_regime, 'general')
        self.assertEqual(self.env['purchase.order'].datacil_check_partner('0000000000')['code'], 'invalid')
