...
export function useDatacilLookup() {
    ...
    async function run({ record, method, query, apply = true, onApplied }) {
        ...
        // Only fields known by the current view can be updated client side,
        // date/datetime values must be luxon objects (the server sends strings)
        // and many2one values are `[id, display_name]` pairs in Odoo 18.
        const values = {};
        for (const [name, value] of Object.entries(result.values || {})) {
            ...
            if (value && field.type === "many2one" && !Array.isArray(value) && typeof value === "object") {
                values[name] = [value.id, value.display_name];
            } else if (value && field.type === "datetime" && typeof value === "string") {
