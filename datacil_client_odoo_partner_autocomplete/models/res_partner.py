...
class ResPartner(models.Model):
    ...
    # Many2one fields the native autocomplete widget converts to ``[id, name]`` itself.
    _AUTOCOMPLETE_M2O_FIELDS = ('country_id', 'state_id', 'industry_id')
    ...
    def enrich_by_duns(self, duns, timeout=15):
        ...
        # Odoo 18 only sends the ``duns`` of the suggestion: Datacil suggestions
        # carry the cédula / RUC there (10 or 13 digits, real DUNS have 9).
        api_model = self.env['datacil.api']
        if duns and api_model.is_configured() and api_model.identification_kind(duns) \
                and api_model.normalize_identification(duns) == str(duns).strip():
            return self._datacil_enrich_suggestion(duns)
    ...
    def _datacil_enrich_suggestion(self, identification):
        ...
        values = {}
        for name, value in web_values.items():
            field_type = self._fields[name].type
            if field_type in ('date', 'datetime'):
                continue
            if field_type == 'many2one' and isinstance(value, dict) and name not in self._AUTOCOMPLETE_M2O_FIELDS:
